...
def getWordScore(word, n):
    ...
    wordScore = 0
    for letter in word:
        wordScore += SCRABBLE_LETTER_VALUES.get(letter, 0)
...
def isValidWord(word, hand, wordList):
    ...
    for letter in wordFrequency.keys():
        if (letter in handKeys) and (wordFrequency[letter] <= hand[letter]):
            continue
        else:
            return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand):
        ...
        handPrint = ""
        for letter in hand.keys():
            handPrint += (letter + " ")*hand[letter]
        print("Current Hand:", handPrint)
...
def playGame(wordList):
    ...
    hand = ''
    n = HAND_SIZE
    #ask for user choice while it is not valid
    while True:
        userChoice = input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        #check if user choice is not valid
        if userChoice not in 'nre':
            print("Invalid command.")
            continue

        #if new hand, deal a new hand of size HAND_SIZE and start game
        if userChoice == 'n':
            hand = dealHand(n)
            #start game
            playHand(hand, wordList, n)

        #if r play last hand, if exists
        if userChoice == 'r':
            if not hand:
